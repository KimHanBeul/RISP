...
def dispatch_openai_requests(messages_list, model, temperature, max_tokens=512, top_p=0.9):
    response_contents = [[] for _ in range(len(messages_list))]
    # import pdb; pdb.set_trace()
    for i, message in enumerate(messages_list):
        messages = [
            {
                "role": "assistant",
                "content": "You are a helfpul assistant for generate recommendation. Only rank the given Candidate Movies. Do not generate any explanation for answer. Split your output with line break."
            },
            {
                "role": "user",
        ...
        result = model.chat_completion(dialogs=[messages], temperature=temperature, top_p=top_p)
        output = result[0]['generation']['content']
        response_contents[i] = output

    return response_contents

def summarize_rep_item(messages_list, model, temperature, max_tokens=512, top_p=0.9):
    messages = [
        {
            "role": "assistant",
            "content": "You are a helfpul assistant for summarize the user preference from given items. Do NOT generate any explanation for answer. Present your response in the format below: {Game preference: , liked genres: }"
        },
        {
            "role": "user",
...
class Rank(SequentialRecommender):
    ...
    def load_emb(self):
        # item_emb = self.sentence_model.encode(self.item_text, show_progress_bar = False)
        item_emb_file = os.path.join(self.config['data_path'], f"item_emb_m_2_1.pt")
        item_emb = torch.load(open(item_emb_file, 'rb'))
    ...
    def load_text(self):
        ...
        if self.dataset_name in ['ml-1m', 'ml-01m']:
            ...
            item_text_path = os.path.join(self.config['data_path'], f"item_text.pickle")
            pickle.dump(item_text, open(item_text_path, 'wb'))
    ...
    def get_batch_inputs(self, interaction, idxs, i):
        ...
        return user_his_text, user_his[i % origin_batch_size], candidate_text, candidate_text_order, candidate_idx
    ...
    def dispatch_openai_api_requests(self, prompt_list, batch_size):
        ...
        for i in tqdm(range(0, batch_size, self.api_batch)):
            ...
            #             dispatch_openai_requests(prompt_list[i:i+self.api_batch], self.generator, self.temperature)
            #             # dispatch_openai_requests(prompt_list[i:i+self.api_batch], self.api_model_name, self.temperature)
            #         )
            openai_responses += dispatch_openai_requests(prompt_list[i:i+self.api_batch], self.generator, self.temperature)
            # while True:
            #     try:
            #         openai_responses += asyncio.run(
            #             dispatch_openai_requests(prompt_list[i:i+self.api_batch], self.api_model_name, self.temperature)
    ...
    def prompt_func(self, dataset_name, user_his_text, user_rep_sum_text, candidate_text_order, recall_budget, demo_elems):
        if dataset_name in ['ml-1m', 'ml-01m']:
            ...
            # demo_prompt = "The User's Movie Profile:\n- Watched Movies: {}\n\nUser Preference Information:\n- {}\n\nThe User's Potential Matches:\n- Candidate Movies: {}\n\nBased on the user's watched movies, please rank the candidate movies \\nthat align closely with the user's preferences. \n- You ONLY rank the given Candidate Movies.\n- You DO NOT generate movies from Watched Movies.\n\nPresent your response in the format below:\n1. [Top Recommendation (Candidate Movie)]\n2. [2nd Recommendation (Candidate Movie)]\n...\n20. [20th Recommendation (Candidate Movie)]\n\nAnswer:{}"

            demo_elem_str = ""
            input_1 = demo_elems[0]
            input_2 = [str(j) + '. ' + demo_elems[1][j] for j in range(len(demo_elems[1]))]
            demo_predict = '\n'.join([str(j + 1) + '. ' + demo_elems[2][j] for j in range(len(demo_elems[2]))])
            demo_prompt_str = demo_prompt.format(input_1, demo_elems[3], input_2, demo_predict)
    ...
    def generate_rep_sum_text(self, dataset_name, user_item_list, user_his=None):
        no_num_user_seq = [item.split('. ', 1)[1] for item in user_item_list]

        target_emb = self.sentence_model.encode(' [SEP] '.join(no_num_user_seq), show_progress_bar = False)
        item_embeds = self.sentence_model.encode(no_num_user_seq, show_progress_bar = False)
        rep_item_sim = self.sentence_model.similarity(target_emb, item_embeds)
        top_k_rep_item = 5
        if top_k_rep_item >= len(user_item_list):
            top_k_rep_item = int(len(user_item_list) / 2)
        _, topk_item_idx = torch.topk(rep_item_sim, top_k_rep_item)
        # topk_item_idx = topk_item_idx

        rep_item_text = []
        # ====== top 5 representative item ======
        for i, idx in enumerate(topk_item_idx[0]):
            rep_item_text.append(str(i+1) + '. ' + no_num_user_seq[idx])

        if dataset_name == 'ml-1m':
            rep_sum_text = "Summarize the user preference based on the given movies.\n" + ', '.join(rep_item_text) + '\nPresent your response in the format below: {Liked genres: }'
        elif dataset_name == 'Games':
            rep_sum_text = "Summarize the user preference based on the given games.\n" + ', '.join(rep_item_text) + '\nPresent your response in the format below: {Liked genres: }'
        elif dataset_name == 'lastfm':
            rep_sum_text = "Summarize the user preference based on the given music artists.\n" + ', '.join(rep_item_text) + '\nPresent your response in the format below: {Music artists preference: , liked genres: }'

        rep_sum_text = summarize_rep_item(rep_sum_text, self.generator, self.temperature)

        rep_item = []
        if user_his is not None:
            rep_item = user_his[topk_item_idx]

        return rep_sum_text, rep_item

    def subgraph(self, global_graph, seqs):
        # make sub-graph
        inputs_nodes = set()
        for seq in seqs:
            inputs_nodes.update(seq)

        inputs_nodes = list(inputs_nodes)
        inputs_nodes = np.array(inputs_nodes)
        batch_graph = nx.subgraph(global_graph, inputs_nodes)
        return batch_graph
    
    def graph_retrieval(self, target, subgraph, global_graph, target_seq_emb, k_hop=1, alpha=0.3):
        ...
        for tar in target:
            ...
            for i in range(k_hop):
                all_node_neighbors = []
                for cur_node in hop_iterate:
                    cur_node_neighbors = []
                    neighbors = np.array([[src, graph.nodes(data='weight')[src]] for _, src in graph.out_edges(cur_node)])

                    # filtering by graph weight
                    # graph_weigth_topk = int(len(neighbors) / 2)
                    graph_weigth_topk = 60
                    neighbors = neighbors[np.argsort(neighbors[:, 1])[::-1]]
                    neighbors = neighbors[:graph_weigth_topk]

                    # filtering by text similarity
                    text_sim_topk = 30
                    if len(neighbors) < text_sim_topk:
                        text_sim_topk = len(neighbors)
                    neighbors_text_emb = self.item_emb[neighbors[:, 0].astype(int)]
                    cur_node_text_emb = self.item_emb[cur_node]
                    node_item_sim = F.cosine_similarity(cur_node_text_emb.unsqueeze(0), neighbors_text_emb)
                    # retrieve by target_seq_emb
                    # node_item_sim = F.cosine_similarity(target_seq_emb.unsqueeze(0), neighbors_text_emb)

                    _, topk_item_idx = torch.topk(node_item_sim, text_sim_topk)
                    topk_item_idx = topk_item_idx.cpu().numpy()
                    top_k_node = neighbors[topk_item_idx, 0]

                    for node in top_k_node:
                        cur_node_neighbors.append(int(node))
                        all_node_neighbors.append(int(node))
                    ...
                    # node_list[cur_node] = cur_node_neighbors
                    node_list.append([cur_node, cur_node_neighbors])
                hop_iterate = all_node_neighbors

        return node_list

    def predict_on_subsets(self, interaction, idxs, global_graph, train_data_seq, train_seq_emb, test_seq_emb, valid_data, selected_uids):
        # demo_elems = self.demo_construction(valid_data, selected_uids)
        # valid_data.dataset['user_id', 'item_id_list', 'item_id', 'item_length']
        ...
        for i in tqdm(range(batch_size)):
            user_his_text, user_his, candidate_text, candidate_text_order, candidate_idx = self.get_batch_inputs(interaction,
                                                                                                        idxs, i)
            
            # ================== Local Graph Construction ==================
            target_emb = test_seq_emb[i]
            similarity = F.cosine_similarity(target_emb.unsqueeze(0), train_seq_emb)
            _, topk_seq_idx = torch.topk(similarity, 25)
            topk_seq_idx = topk_seq_idx.cpu().numpy()

            semantic_sim_seq = train_data_seq['item_id_list'].loc[topk_seq_idx].tolist()
            semantic_sim_seq.append(user_his.tolist())
            sub_graph = self.subgraph(global_graph, semantic_sim_seq)
            # ==============================================================

            # # ================== Demonstration Construction ==================
            # demo_elem = [[] for _ in range(4)]
            # demo_elem[0] = user_his_text[:-1]   # item_id_list
            # random.seed(self.config['seed'])
            # # random_can_idx = np.random.choice(np.arange(1, len(self.item_text)), self.recall_budget-1, replace=False).tolist()
            # random_can_idx = random.sample(range(1, len(self.item_text)), self.recall_budget - 1)
            # for idx in random_can_idx:
            #     demo_elem[1].append(self.item_text[idx])
            #     demo_elem[2].append(self.item_text[idx])
            
            # random.seed(self.config['seed'])
            # random.shuffle(demo_elem[2])

            # demo_elem[1].insert(0, user_his_text[-1])   # candidate item set
            # demo_elem[2].insert(0, user_his_text[-1])   # answer
            # # ================================================================

            # # ================== Representative Item Selection ==================
            # demo_elem[3], _ = self.generate_rep_sum_text(self.dataset_name, demo_elem[0])
            # target_rep_sum, rep_item = self.generate_rep_sum_text(self.dataset_name, user_his_text, user_his)
            # # demo_elem= demo_elems[i]
            # # ===================================================================

            # cans_seq = self.graph_retrieval(rep_item[0].tolist(), sub_graph, global_graph)
            # cans_seq = self.graph_retrieval(user_his[user_his.nonzero(as_tuple=True)[0][-5:]].tolist(), sub_graph, global_graph, target_emb)
            cans_seq = self.graph_retrieval(user_his[user_his.nonzero(as_tuple=True)[0][-5:]].tolist(), None, global_graph, target_emb)

            for cur_node, neighbors in cans_seq:
                target_item = int(pos_items[i % origin_batch_size])
                if target_item in neighbors:
                    print("Test Case: {}, Found ground truth item!".format(i))
                    test_case_num.append(i)
                    break

            # input_ = self.prompt_func(self.dataset_name, user_his_text, target_rep_sum, candidate_text_order,
        ...
        print("Ground truth founded test case: {}\n# of Ground Truth Founded: {}".format(test_case_num, len(test_case_num)))
